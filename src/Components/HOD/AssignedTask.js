...
async componentDidMount(){
    await axios.get('http://localhost:3306/AssignedTaskHOD', { headers: {
...
async approveRow(id){
    try {
        ...
        var res = await axios({
            method: 'post',
            url: 'http://localhost:3306/AssignedTaskHOD/approveTask',
            data: {
                taskId: taskId,
            }
        })
...
discardRow(id) {
    Swal.fire({
        ...
        preConfirm: async (t) => {
            await axios.post(`http://localhost:3306/AssignedTaskHOD/discardTask`, { taskId: id })
            .then(res =>{
            }, err => {
            })
        }
      }).then(async function (t) {
        await axios.post(`http://localhost:3306/AssignedTaskHOD/postReview`, { taskId: id, review: t.value, user: localStorage.getItem('userId'), role: localStorage.getItem('roleId') })       
        .then(res =>{
            if(res.data.success){
