...
Router.get('/', function (req, res, next) {
    ...
    // console.log(userId)
    db.query('SELECT idTask,Description,StatusName,AssignDate,Deadline,uploadFile,Comment, users.Name FROM task, users, status WHERE task.Users_idUser= users.idUser and task.Status = status.StatusId', (err, data)=>{
        if (err) {
            res.json({
                success: false,
